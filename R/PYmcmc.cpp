...
double prob_existing(const int k, const int l, const vec group_t, const mat mu_t, const mat cov_t,
                     const vec y, const umat G, const double f, const double delta){
  double result = 0;
  uvec index = find(group_t == k);
  int n_k;
  if(group_t(l) == k){
  ...
  result += log(1+n_k - delta) + MRF_energy(k, l, group_t, G, f);
...
double prob_new(const int K, const vec y, const mat cov_t, const double GAMMA, 
  ...
  double result = max((h + log(GAMMA - K *0.03) ), -999999.0);
  return result;
}
...
void update_group(vec &group_t, mat &mu_t, mat &cov_t, const mat &Y_t, 
                  ...
                  const double &delta, const double &tau, const double &temperature){
  ...
  for(int i = 0; i < N; i++){
    ...
    if(c_size > 1){
      ...
      for(int k = 0; k < K; k++){
        prob_i(k) = prob_existing(k, i, group_t, mu_t, cov_t, Y_t.col(i), G, f, delta);
      }
...
// [[Rcpp::export]]
Rcpp::List PYmcmc(vec &group_t, mat &W_t, vec &var_inv_t, mat &Y_t, mat &mu_t, mat &cov_t, mat &cov_inv_t, 
  ...
  for(int t = 1; t < max_iters; t++){
    ...
    update_group(group_t, mu_t, cov_t, Y_t, Kmax, G, f, GAMMA, delta, tau, temperature);
