...
double prob_new(const int K, const vec vn, const vec y, const mat cov_t, const double GAMMA, 
                const double tau){
  int q = y.n_elem;
  vec mu = zeros<vec>(q);
...
void update_group(vec &group_t, mat &mu_t, mat &cov_t, const mat &Y_t, 
                  const vec &vn, const umat &G, const double &f, const double &GAMMA, 
  ...
  for(int i = 0; i < N; i++){
    ...
    if(c_size > 1){
      ...
      prob_i(K) = prob_new(K, vn, Y_t.col(i), cov_t, GAMMA, tau);
...
// [[Rcpp::export]]
Rcpp::List MFMmcmc(vec &group_t, mat &W_t, vec &var_inv_t, mat &Y_t, mat &mu_t, mat &cov_t, mat &cov_inv_t, 
  ...
  for(int t = 1; t < max_iters; t++){
    ...
    update_group(group_t, mu_t, cov_t, Y_t, vn, G, f, GAMMA, tau, temperature);
