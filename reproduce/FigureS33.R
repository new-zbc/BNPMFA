...
library(ggplot2)
sampleID = 151507
df = read.csv(file = paste0("application/DLPFCdata/", sampleID, "/nHVG_explore.csv"))
p1 = ggplot(df, aes(x = eps, y = ARI)) + 
  geom_line()+geom_point() + theme_bw() + xlab("Number of HVGs")+
  ylab("ARI") + labs(title = sampleID)+ ylim(c(-0.1,1))+
...
ggsave(p, filename = "reproduce/img/FigureS33.pdf", width = 10, height = 5, bg = "white")